...
class Dataset(object):
    def __init__(self, lst_sub_faces, batch_size, num_frames, label,seed=66):
        self.lst_sub_faces = torch.unbind(lst_sub_faces.view(-1, num_frames, 512))
        ...
        for i in range(len(self.lst_sub_faces)):
            idx = np.random.permutation(np.size(self.lst_sub_faces[i], 0))
